...
test('pakiet dystrybucyjny: artefakt + drzewo map + ZIP (ADR 0027 v2)', async () => {
  ...
  if (maRasterizer) {
    ...
    assert.ok(fs.existsSync('/tmp/codex-test-pakiet/maps/srodziemie/podklad.svg'), 'pakiet: bez rasterizatora zostaje pełna baza Śródziemia');
  }
  ...
  // ADR 0027 v3: przy dostępnym rasterizatorze drzewo niesie mini.jpg i nie
  // duplikuje baz SVG; w środowisku bez niego build legalnie spada na pełną bazę.
  ...
  for (const f of pliki.filter((p) => p.endsWith('.html'))) {
    ...
    if (maRasterizer) assert.ok(fs.existsSync(mini), `mini-mapa planu: ${mini}`);
    if (fs.existsSync(mini)) assert.ok(fs.statSync(mini).size <= 400 * 1024, `${mini} ≤ 400 kB (screenshot, nie cała baza)`);
  }
  if (maRasterizer) assert.ok(index.includes('maps/lorwyn/mini.jpg'), 'CODEX_DATA: mini-mapa Lorwyn z mini.jpg');
